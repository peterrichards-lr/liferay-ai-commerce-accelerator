...
20. **Fix client extension permissions inside container**: Run `chown` and `chmod` via `docker exec` in `run-e2e-ldm.sh` immediately after `ldm deploy` to ensure the `liferay` user can read and deploy the ZIP files on Linux host runners. [Completed]
