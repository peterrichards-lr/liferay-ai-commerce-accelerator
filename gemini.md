...
#### AI Schemas Panel Refactor

The AI Schemas configuration screen (`ai-commerce-accelerator-configuration`) was updated to simplify how schema editors are defined and rendered. Previously, each schema (Product, Account, Order) was hard‑coded individually in the component. This resulted in duplicated logic for loading, validating, saving, and rendering schemas.

To improve maintainability and extensibility, the implementation was refactored to use a single `ENTITY_CONFIGS` array that defines each schema’s `id`, `title`, and `configKey`. All logic now iterates over this configuration array, allowing new schema types to be added by inserting one configuration object rather than rewriting UI and persistence logic. This significantly reduces repeated code and prevents divergence of logic across entity types.

Key benefits:

* Centralised configuration for all schema types.
* Automatic support for loading, editing, validation, rendering, and saving through array iteration.
* Eliminates boilerplate and reduces the chance of introducing bugs when adding new schema types.
* Removes reliance on Clay tabs, which caused CodeMirror rendering issues; schemas are now listed vertically for better stability.

---

### Prompt Configuration Components

The application includes a set of Prompt configuration components within the configuration UI. These components are responsible for allowing administrators to define how AI prompting should behave for the microservice.

#### Purpose of Prompt Configuration Components

Prompt configuration components allow administrators to:

* Define system prompts or instructions used by the microservice when generating commerce‑related data (products, accounts, orders, images, relationships, descriptions, and more).
* Provide templates that the microservice fills with contextual data at runtime.
* Adjust prompting behaviour without rebuilding or redeploying the microservice.
* Ensure consistency across AI‑generated entities.

These components are intentionally decoupled from the frontend accelerator UI. The microservice reads the stored prompt configuration from Liferay’s configuration store (`persistConfigKey` + `getKeyValue`) and uses it to control model behaviour.

#### How Prompt Configuration Should Be Used in Code

Prompt configuration should follow these principles:

1. **Single Source of Truth**  
   All prompt strings and templates should be retrieved through the configuration UI’s persisted configuration. Hardcoding prompts inside the microservice should be avoided unless providing a safe fallback.

2. **Runtime Injection**  
   During request handling, the microservice should fetch the appropriate prompt configuration and inject relevant data into the template before sending it to the model.

3. **Dynamic Extendability**  
   Prompt categories (e.g., product generation prompt, specification prompt, taxonomy prompt) should map cleanly to configuration keys so the configuration UI can be extended in the same way as the AI Schema Panel uses `ENTITY_CONFIGS`.

4. **Validation and Error Messaging**  
   The configuration UI should ensure that prompt fields contain usable text (not empty unless intentionally blank), while the microservice should fail gracefully and report meaningful errors when prompts are missing.

5. **Separation of Concerns**  
   The accelerator frontend interacts only with the microservice for prompt‑driven operations. It does not read prompt configuration directly; this keeps the architecture modular and scalable.

---

### How Prompt Configuration Is Used Inside the Microservice

* The microservice reads configuration keys during runtime using the configuration helper (`config-node`).
* Prompt strings are passed into model calls as part of the system or user message payloads.
* If a prompt defines placeholders (e.g., `{{name}}`, `{{sku}}`, `{{attributes}}`), the microservice is responsible for filling these before calling the model.
* If the configuration is blank or malformed, the microservice should fallback to safe defaults or return a descriptive error message to the client.

These conventions help ensure the AI behaviour is predictable, modifiable, and resilient.
