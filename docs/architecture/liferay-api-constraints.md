...
- **A run may skip pricing entirely**: because the write is unconditional and `create-product-skus` sends a price on every SKU, a run with **Generate Price Lists** off still leaves every generated SKU with a resolvable price in `<catalog> Base Price List`. The toggle governs AICA's own price-entry writes (`create-price-lists`, and the bulk and tier steps nested under it), not whether the catalogue is priced. `update-catalog-config` stays outside the toggle: it is the only step that moves the `catalogBasePriceList` flag back onto the catalog's own list when an earlier run left it on an AICA one, and that flag is what decides where `Sku.price` lands.
