...
- **Option 1: JAX-RS REST Endpoint (`/o/search-reindex`)**: A lightweight Java OSGi module exposing a secure REST API `/o/search-reindex/reindex/{className}` (restricted to Omniadmins), facaded by the JS SDK (`triggerReindex()`) and automatically invoked. The module is **not built here** - it lives in [`liferay-custom-osgi-modules`](https://github.com/peterrichards-lr/liferay-custom-osgi-modules) as `com.liferay.custom.search.reindex` and is consumed through the pinned release in `gradle.properties`, so that other projects can use it too. It was formerly `/o/aica-reindex` in this repository; the base and the OAuth scope (`Custom.Search.Reindex.everything.write`) were both genericised in the move:
