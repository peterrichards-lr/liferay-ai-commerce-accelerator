...
- **Example**:
  ...
  query {
    headlessCommerceAdminCatalog_v1_0 {
      a0: productByExternalReferenceCode(externalReferenceCode: "ERC-1") {
        id
        name
      }
      a1: productByExternalReferenceCode(externalReferenceCode: "ERC-2") {
        id
        name
      }
    }
...
_(and all other internal paths...)_
...
_(and all other Liferay paths...)_
...
- **`parallel`**: A container step that holds an array of sub-steps. All sub-steps within a `parallel` block are initiated concurrently. The `parallel` step itself is considered complete only when _all_ its sub-steps have completed.
...
- **`_checkSessionCompletion` method**:
  ...
  - **Asynchronous Steps**:
    ...
    - Immediately advance the workflow to the _next_ top-level step without waiting for the `async` step's completion.
...
4.  **Path and Parameter Constraints**:
    ...
    - **Batch URL Redundancy**: Avoid passing parent identifiers (like `priceListId`) as query parameters to nested batch endpoints (e.g., `/price-lists/price-entries/batch`). Liferay often interprets these as identifiers for the _ImportTask_ object itself, leading to ERC collisions ("external reference code already in use").
...
1.  **Persistence-First Submission**: Batch metadata (ERC, Session ID, Step Key) MUST be persisted to the local database _before_ the batch is submitted to Liferay. This ensures that if a callback arrives extremely quickly (even before the submission call returns), the handler can correlate it.
