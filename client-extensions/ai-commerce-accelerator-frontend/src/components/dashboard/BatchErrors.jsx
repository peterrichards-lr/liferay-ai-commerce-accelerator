  ...
  if (!Array.isArray(batchErrors) || batchErrors.length === 0) {
    return (
      <div className="empty-errors-state mt-4 text-center py-5">
        <ClayIcon symbol="check-circle" className="text-success mb-3" style={{ fontSize: '3rem' }} />
    ...
    return (
      <div className="mt-4">
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <ClayButton onClick={() => clearBatchErrors(entityFilter)} displayType="secondary" small>
            Back to All
          </ClayButton>
        </div>
        <p className="text-muted">No errors found for this specific category.</p>
      </div>
    );
  }
      ...
      {filteredErrors.map((error, index) => (
        ...
        >
          <div className="card-header d-flex justify-content-between align-items-center">
            ...
            <span className="label label-warning">{error?.entityType || 'unknown'}</span>
          </div>
          <Card.Body>
            ...
            <div className="failed-items-section">
              ...
              {Array.isArray(error?.errorReport) && error.errorReport.length > 0 ? (
                <div className="table-responsive">
                  <table className="table table-autofit table-list table-nowrap table-sm">
                    <thead>
                      <tr>
                        <th className="table-cell-expand">Index</th>
                        <th className="table-cell-expand">External Reference Code</th>
                    ...
                    <tbody>
                      {error.errorReport.map((item, itemIndex) => (
                        <tr key={item?.itemIndex ?? itemIndex}>
                          <td className="table-cell-expand">{item?.itemIndex}</td>
                          <td className="table-cell-expand">
                            <code>
                              {item?.externalReferenceCode || 
                               (item?.item && (() => {
                                  try {
                                    return JSON.parse(item.item)?.externalReferenceCode;
                                  } catch { return 'n/a'; }
                               })()) || 'n/a'
                              }
                            </code>
                ...
                <p className="mb-0 text-muted italic">
                  No individual item reports available for this batch. Check microservice logs for details.
                </p>
              )}
            </div>
          </Card.Body>
