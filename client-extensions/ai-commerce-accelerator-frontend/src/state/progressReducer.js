...
export function progressReducer(state, action) {
  switch (action.type) {
    // Back-compat: apply an updater function(state) => newState
    case 'APPLY_UPDATER': {
      const next = action.updater(state);
      return next ?? state;
    }

    // Back-compat: shallow-merge a partial state object
    case 'MERGE': {
    ...
    case 'INCR_COMPLETED': {
      ...
      const cur = state[entity] || { total: 0, completed: 0, errors: [] };
      return { ...state, [entity]: { ...cur, completed: cur.completed + amount } };
    }
    ...
    case 'ADD_ERRORS': {
      ...
      return { ...state, [entity]: { ...cur, errors: [...cur.errors, ...add] } };
    }
    ...
    case 'RESET_ALL': {
      ...
      for (const [entity, total] of Object.entries(withTotals)) {
        if (!base[entity]) base[entity] = { total: 0, completed: 0, errors: [] };
