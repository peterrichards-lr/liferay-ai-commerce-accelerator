...
function loadEnv() {
  ...
  if (!fs.existsSync(envPath)) {
    console.warn(`${YELLOW}⚠️ Warning: No .env file found in workspace root. Using process environment variables.${RESET}\n`);
    return process.env;
  }
...
async function runPreflight() {
  console.log(`${BOLD}🛡️  Liferay AI Commerce Accelerator Sentinel — Pre-flight Checks${RESET}\n`);

  const env = loadEnv();

  // 1. Validate Essential Env Variables
  console.log('📋 Validating required environment variables...');
  const liferayUrl = env.LIFERAY_API_URL || env.LIFERAY_URL || 'http://localhost:8080';
  ...
  if (!liferayUsername || !liferayPassword) {
    console.error(`❌ [${RED}ERROR${RESET}] Missing Liferay connection credentials in .env.`);
    console.error(`          Ensure both ${BOLD}LIFERAY_API_USERNAME${RESET} and ${BOLD}LIFERAY_API_PASSWORD${RESET} are set.`);
  ...
  const aiKeys = [env.OPENAI_API_KEY, env.GEMINI_API_KEY, env.ANTHROPIC_API_KEY, env.AI_API_KEY];
  const hasAIKey = aiKeys.some(key => key && key.trim() !== '');
  if (!hasAIKey) {
    console.warn(`   ${YELLOW}⚠️ Warning: No AI provider keys found in .env (OpenAI, Gemini, Anthropic).`);
    console.warn(`              AI features will fall back to disconnected mock states.${RESET}`);
  } else {
  ...
  for (const port of ports) {
    ...
    if (isBound) {
      console.error(`❌ [${RED}ERROR${RESET}] Port ${BOLD}${port.num}${RESET} (${port.name}) is already in use.`);
      console.error(`          Please kill the stale process running on port ${port.num} before starting.`);
      hasCriticalFailure = true;
    } else {
  ...
  console.log(`🌐 Verifying Liferay DXP connectivity to ${BOLD}${liferayUrl}${RESET}...`);
  const isLiferayOnline = await checkLiferay(liferayUrl);
  if (!isLiferayOnline) {
    console.warn(`   ${YELLOW}⚠️ Warning: Cannot connect to Liferay at ${liferayUrl}`);
    console.warn(`              Please ensure Liferay is started and running, or verify your network routing.`);
    console.warn(`              E2E tests and live API requests will fail if offline.${RESET}`);
  } else {
  ...
  if (hasCriticalFailure) {
    ...
    console.error(`   Please address the errors listed above before launching the stack.\n`);
    process.exit(1);
  } else {
