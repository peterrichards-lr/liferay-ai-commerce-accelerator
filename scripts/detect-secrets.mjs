...
const SECRET_PATTERNS = [
  {
    ...
    regex: /(?:key|secret|password|passwd|token|auth|credential|private_key)\s*[:=]\s*["'](?![^"']*(?:test|mock|dummy|example|localhost|api|user|email|auth|token|cb|db|uuid|path|en_US|null|undefined|http))[^"']{8,}["']/i
  },
  {
    name: 'OpenAI API Key',
...
function checkSecrets() {
  console.log('🔒 Running Node-native Secrets Leak Detection check on staged files...');

  let stagedFilesText;
  try {
    stagedFilesText = execSync('git diff --cached --name-only --diff-filter=ACM', { encoding: 'utf8' });
  } catch (err) {
    console.error('⚠️ Failed to list git staged files:', err.message);
    process.exit(0); // Pass gracefully if git is not available
  }

  const files = stagedFilesText.split('\n').map(f => f.trim()).filter(Boolean);
  if (files.length === 0) {
    ...
    lines.forEach((line, index) => {
      ...
      SECRET_PATTERNS.forEach(pattern => {
        ...
        if (match) {
          ...
          console.error(`   💡 If this is a safe false-positive, append this comment at the end of the line: // ${IGNORE_PRAGMA}`);
          leaksFound++;
        }
      });
    });
